...
def run_playwright_isolated_v2(job_id, llave, urlproceso_limpia, raw_dir, zip_path,
                             max_retries, state_file, main_loop, log_queue,
                             active_cancellations, descargar_archivos):
...
async def download_pdfs_for_contract_v2(job_id: str, llave: str, urlproceso, log_queue: asyncio.Queue,
                                      max_retries: int = 3, active_cancellations: set = None, descargar_archivos: bool = True) -> dict:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        await main_loop.run_in_executor(
            executor,
            run_playwright_isolated_v2,
            job_id, llave, urlproceso_limpia, raw_dir, zip_path, max_retries,
            STATE_FILE, main_loop, log_queue, active_cancellations or set(), descargar_archivos
        )
        ...
        for file_name in os.listdir(raw_dir):
            ...
            if file_path.is_file():
                ...
                if descargar_archivos:
                    ...
                    sha256_pdfs[file_name] = "Hash no disponible - Documento excluido de descarga local"
